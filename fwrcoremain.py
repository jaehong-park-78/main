```python
"""
FWR Stability Controller v3.8.2
- Cascade 감지 데모 로직 수정
- R을 학습 루프 실측값 기반으로 동적 계산
  (loss, grad_norm을 실제 학습 중 측정)
"""
...
# ============================================================
# 0. 데이터셋 (F/W용 보상 신호만 — R은 학습 루프에서 실측)
# ============================================================
...
class FWRStabilityController(nn.Module):
    ...
    def forward(self, f_tensor, w_tensor, r_tensor):
        ...
        # 2. 창발 에너지: E = F × W × R (FWR 원칙)
        e_tensor = f_tensor * w_tensor * r_adj

        # 3. resonance_quality
        # 배치 크기 1이면 std=0 경고 발생 → unbiased=False로 방지
        if r_tensor.numel() > 1:
    ...
    def detect_resonance_cascade(self, current_r_tensor=None):
        """
        공명 붕괴 + 과잉 폭주 감지.
        current_r_tensor: 현재 배치 실시간 텐서 (링 버퍼 지연 제거)
        """
        if not self.history_initialized.item():
            return False

        ptr = self.history_ptr.item()
        # 히스토리가 최소 2개는 있어야 velocity 계산 가능
        if ptr < 2:
            return False
...
def demo_resonance_conversion():
    """위험 신호 → 공명 강도 변환 검증"""
    print("\n" + "=" * 70)
    print("ResonanceFromRiskInterface: 위험 → 공명 변환 (v3.8.2)")
    ...
    print("-" * 60)
    print("기대: 위험 높을수록 R 낮음")


def demo_fwr_formula():
    """E = F × W × R 수식 검증"""
    print("\n" + "=" * 70)
    print("E = F × W × R 수식 검증 (v3.8.2)")
    print("=" * 70)
    ...
    print("-" * 55)
    print("기대: 위험 높을수록 R 낮음 → E 낮음")


def demo_cascade_detection():
    """공명 붕괴 + 과잉 폭주 감지 (수정된 데모 로직)"""
    print("\n" + "=" * 70)
    print("Cascade 감지 (v3.8.2)")
    print("=" * 70)
...
# ============================================================
# 8. 통합 학습 루프 v3.8.2
#    R을 학습 루프 실측값으로 동적 계산
# ============================================================
def train_fwr_agi_v382(
    agi_model,
    fwr_controller,
    alpha_scheduler,
    reward_interface,
    ...
    v3.8.2 핵심:
    - R을 매 배치 실측값으로 동적 계산
      (task_loss: 현재 배치 MSE, grad_norm: 현재 파라미터 gradient norm)
    - target_e_value=None이면 첫 배치 E 평균으로 자동 설정
    """
    ...
    print("\n" + "=" * 70)
    print("FWR AGI Training v3.8.2 (R = 실측 기반 동적 계산)")
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            # 4. R 사전 계산용 임시 forward (gradient 없이)
            with torch.no_grad():
                # task_loss: 현재 F, W로 만든 E와 임시 target의 차이
                E_temp = F_combined * W_combined  # 단순 곱으로 proxy
                task_loss_proxy = torch.mean((E_temp - task_reward) ** 2).unsqueeze(0).unsqueeze(0)
                task_loss_proxy = task_loss_proxy.expand(inputs.shape[0], 1)

                # grad_norm: 이전 스텝의 파라미터 grad norm (없으면 0)
                grad_norms = []
                ...
                grad_norm_tensor = torch.full(
                    (inputs.shape[0], 1), current_grad_norm, device=inputs.device
                )
...
def test_trained_model(agi_model, fwr_controller, resonance_interface, fusion_scheduler):
    ...
    with torch.no_grad():
        ...
        print(f"F (추진력):\n{F}")
        print(f"W (구조력):\n{W}")
        print(f"R (공명-변환): mean={R.mean().item():.4f}")
        print(f"R (공명-조정): mean={R_adj.mean().item():.4f}")
        print(f"E (창발 에너지):\n{E_out}")
...
if __name__ == "__main__":
    ...
    agi_model, fwr_controller, alpha_scheduler = train_fwr_agi_v382(
